...
# Runs on each ASG instance — LabInstanceProfile gives it PutMetricData permission.
...
# ---------- memory ----------
# CloudWatch has no built-in memory metric and this is the main reason for the script
...
# Read from the systemd service journal — count open WS upgrade lines in the last 60s.
